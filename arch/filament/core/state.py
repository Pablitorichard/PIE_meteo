...
class State():
    ...
    def fromCDF(cls, netCDF_file, k=None):
        ...
        vars = {variable: netCDF_file[variable][:,:,k].data if k is not None else netCDF_file[variable][:].data
                for variable in variables}
        return cls(t, vars)

    @classmethod
    def copy(cls, otherState):
        return cls(otherState.t, otherState.vars)
    ...
    def save(self, netCDF_file, backup=False):
        ...
        for variable in export_variables:
            netCDF_file[variable][:,:,k] = self.vars[variable]