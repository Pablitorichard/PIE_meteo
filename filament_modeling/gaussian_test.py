...
def gaussian_test(path, Lx, Ly, Nx, Ny, T, Nt, cx, cy, radius, wind_norm):
...
#INITIALIZATION ---------------------------------------------------------------
    ...
    F =np.zeros((Nx,Ny)) 
    F[bubble_indices] = np.reciprocal((handle['x_grid'][bubble_indices] - cx)**2 +
                               (handle['y_grid'][bubble_indices] - cy)**2 )
